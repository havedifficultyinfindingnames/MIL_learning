...
theorem neg_neg (a : R) : - -a = a := by
  have : -a + a = 0 := by exact neg_add_self a
  rw [neg_eq_of_add_eq_zero]; apply this
...
theorem mul_inv_rev (a b : G) : (a * b)⁻¹ = b⁻¹ * a⁻¹ := by
  rw [← mul_one (b⁻¹ * a⁻¹), ← mul_right_inv (a * b)]
  rw [← mul_assoc, mul_assoc b⁻¹ a⁻¹];
  rw [← mul_assoc a⁻¹ a b];
  rw [mul_left_inv a, one_mul, mul_left_inv, one_mul]
